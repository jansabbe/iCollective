...
- (void)viewDidLoad {
    ...
    [[ICRestKitConfiguration profilePicQueue] cancelAllRequests];
    [ICRestKitConfiguration fetchImage:self.person.photoUrl delegate:self];
}

- (void)request:(RKRequest *)request didLoadResponse:(RKResponse *)response {
    self.photo.image = [UIImage imageWithData:response.body];
}
